...
API Key Manager for SAJHA MCP Server
...
class APIKeyManager:
    ...
    def __init__(self, config_path: str = None):
        ...
        Args:
            config_path: Path to apikeys.json file
        """
        if config_path is None:
            config_path = Path(__file__).parent.parent / 'config' / 'apikeys.json'
    ...
    def _load_keys(self):
        ...
        with self._lock:
            ...
            try:
                ...
                logger.info(f"Loaded {len(self._apikeys)} API keys")
...
def get_api_key_manager(config_path: str = None) -> APIKeyManager:
    """
    Get the singleton APIKeyManager instance.
    
    Args:
        config_path: Path to apikeys.json (only used on first call)
